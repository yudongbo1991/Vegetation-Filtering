...
def part_generation(num_part, emd_size, N_iter=1000):
    ...
    for i in range(N_iter):
        ...
        optimizer.step()
    import os
    os.makedirs('/model/codebook', exist_ok=True)
    np.save('/model/codebook/optimal_{}_{}.npy'.format(num_part, emd_size), optimal_target.detach().numpy())
...
class get_model(nn.Module):
    def __init__(self, concept_num, num_class, kpt_num, point_num, input_dims=256, emb_dims=256):
        ...
        if os.path.exists('model/codebook/optimal_{}_{}.npy'.format(concept_num * num_class, emb_dims)):
            part_gen = np.load('model/codebook/optimal_{}_{}.npy'.format(concept_num * num_class, emb_dims))
        else:
